...
use halo2_ecc::{
    ...
    fields::{FieldChip, native_fp::NativeFieldChip},
    bigint::{CRTInteger, FixedCRTInteger, ProperCrtUint},
};

// Current crate and module
use crate::loader::{
    evm::{encode_calldata, Address, EvmLoader},
    halo2,
    native::NativeLoader,
    Loader,
};
...
pub fn test_powers (input: (Fq, usize)) -> Fr {
    ...
    let a = chip.base_chip.load_private(builder.main(0), input.0.clone());
    let out = chip.powers_base(builder.main(0), &a, input.1);
...
pub fn test_inner_product (input: (Vec<Fq>, Vec<Fq>)) -> Fr {
    ...
    let a = chip.load_witnesses(builder.main(0), &input.0.clone());
    let b = chip.load_witnesses(builder.main(0), &input.1.clone());
    let out = chip.inner_product_base(builder.main(0), &a, &b);
...
pub fn test_sum (input: Vec<Fq>) -> Fr {
    ...
    let a = chip.load_witnesses(builder.main(0), &input);
    let out = chip.sum_base(builder.main(0), &a);
    *out.unwrap().native().value()
...
pub fn test_product (input: Vec<Fq>) -> Fr {
    ...
    let a = chip.load_witnesses(builder.main(0), &input);
    let out = chip.product_base(builder.main(0), &a);
    *out.unwrap().native().value()
...
pub fn test_lagrange_eval (input: &[Fq]) -> Fr {
    ...
    let input = chip.load_witnesses(builder.main(0), input);
    let a = chip.lagrange_and_eval_base(builder.main(0), &[(input[0].clone(), input[1].clone())], &input[2]);
...
pub fn test_lagrange_eval4 (input: &[Fq]) -> Fr {
    ...
    let input = chip.load_witnesses(builder.main(0), input);
    let a = chip.lagrange_and_eval_base(builder.main(0), &[(input[0].clone(), input[1].clone()),(input[2].clone(), input[3].clone())], &input[4]);
    *a.unwrap().native().value()
...
pub fn test_horner (input: &[Fq]) -> Fr {
    ...
    let input = chip.load_witnesses(builder.main(0), input);
    let a = chip.horner_base(builder.main(0), &[input[0].clone(), input[1].clone(), input[2].clone(), input[3].clone()], &input[4].clone());
    *a.unwrap().native().value()
...
pub fn test_sum_check (input: ( usize, usize, &Fq, Vec<Fq> )) -> ( Fr, Vec<Fr> ) {
    ...
    let a = chip.base_chip.load_private(builder.main(0), input.2.clone());
    let msg = chip.load_witnesses(builder.main(0), &input.3);
    let out = chip.verify_sum_check_base::<true>(builder.main(0), input.0, input.1, &a);
